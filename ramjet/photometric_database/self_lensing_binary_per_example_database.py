...
class SelfLensingBinaryPerExampleDatabase(TessLightcurveLabelPerTimeStepDatabase):
    ...
    def convert_raw_synthetic_data_to_project_format(self):
        ...
        """
        path_to_agnieszka_compressed_data = Path(self.data_directory, 'TrainingSetedgeon_nonorm.tar.gz')
        ...
        with tarfile.open(path_to_agnieszka_compressed_data, "r:gz") as tar_file:
            ...
            Path(self.data_directory, 'TrainingSetedgeon_nonorm').rename(synthetic_data_directory)
    ...
    def download_and_prepare_database(self, number_of_negative_lightcurves_to_download=10000,
        ...
        synthetic_signals_url = ('https://files.slack.com/files-pri/TKNTDRH5J-FRG8RSHR9/download/' +
                                 'trainingsetedgeon_nonorm.tar.gz?pub_secret=6bb12030bb')
        ...
        if magnitude_filter is not None:
            ...
            selected_tic_ids = magnitude_selected_tic_entries['ID'].values
            # Break the query into chunks of 1000 TIC IDs, otherwise MAST complains its too big of a query.
            for tic_ids in np.array_split(selected_tic_ids, math.ceil(selected_tic_ids.shape[0] / 1000)):
                tic_observations = self.get_all_tess_time_series_observations({'target_name': list(tic_ids)})
                if tess_observations is None:
                    tess_observations = tic_observations
                else:
                    tess_observations = tess_observations.append(tic_observations, ignore_index=True)
