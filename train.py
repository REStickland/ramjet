...
from ramjet.photometric_database.microlensing_center_label_time_step_database import \
    MicrolensingCenterLabelPerTimeStepDatabase
...
def train():
    ...
    database = MicrolensingCenterLabelPerTimeStepDatabase()

    # database.batch_size = 100  # Reducing the batch size may help if you are running out of memory.
    epochs_to_run = 1000
    trial_name = 'Center included labeling approach'
    logs_directory = 'logs'
    ...
    try:
        model.fit(training_dataset, epochs=epochs_to_run, validation_data=validation_dataset,
                  callbacks=[tensorboard_callback], validation_freq=1)
